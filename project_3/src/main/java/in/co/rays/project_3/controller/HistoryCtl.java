...
/**
 * History functionality ctl.to perform add,delete ,update operation
 * 
 * @author Shruti Rathore
 *
 */

@WebServlet(urlPatterns = { "/ctl/HistoryCtl" })
public class HistoryCtl extends BaseCtl {

	private static Logger log = Logger.getLogger(HistoryCtl.class);

	protected boolean validate(HttpServletRequest request) {

		log.debug("History ctl validate start");
		...
		if (DataValidator.isNull(request.getParameter("loginTime"))) {
			request.setAttribute("loginTime", PropertyReader.getValue("error.require", "loginTime"));
			pass = false;
		}
		else if (!DataValidator.isDate(request.getParameter("loginTime"))) {
			request.setAttribute("loginTime", PropertyReader.getValue("error.name", " loginTime"));
			pass = false;
		}
		...
		log.debug("History ctl validate end");
	...
	protected BaseDTO populateDTO(HttpServletRequest request) {
		...
		log.debug("History ctl populate bean start");
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("History ctl do get start");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (OP_SAVE.equalsIgnoreCase(op) || OP_UPDATE.equalsIgnoreCase(op)) {
			...
			try {
				model.delete(dto);
				ServletUtility.redirect(ORSView.HISTORY_LIST_CTL, request, response);
				return;
			} catch (ApplicationException e) {
				log.error(e);
				ServletUtility.handleException(e, request, response);
			...
			ServletUtility.redirect(ORSView.HISTORY_CTL, request, response);
			return;

		}
		ServletUtility.forward(getView(), request, response);

		log.debug("History ctl do post end");
